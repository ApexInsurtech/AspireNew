...
import negotiation.contracts.ClaimContract
...
object ClaimFlow {
    ...
    class Initiator(val proposalId: UniqueIdentifier) : FlowLogic<UniqueIdentifier>() {
        ...
        override fun call(): UniqueIdentifier {
            ...
            val inputStateAndRef = serviceHub.vaultService.queryBy<CoverageVerificationState>(inputCriteria).states.single()
            val input = inputStateAndRef.state.data


            val output = ClaimState(input.policy_applicant_name, input.policy_applicant_mailing_address,
                    input.buyer, input.seller, input.linearId)


            // Creating the command.
            val commandType = ClaimContract.Commands.Propose()
            ...
            txBuilder.addOutputState(output, ClaimContract.ID)
            ...
            return finalisedTx.tx.outputsOfType<ClaimState>().single().linearId
        }
    }
