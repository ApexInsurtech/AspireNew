...
@InitiatingFlow
@StartableByRPC
class AddChildPolicyFlow(val groupID: String, val member: Party) : FlowLogic<UniqueIdentifier>() {
    /**
    ...
    override fun call(): UniqueIdentifier {
        ...
        val memberStateState: ChildPolicyState = ChildPolicyState(party = member, moderator = gameState.moderator)
        val notary = this.serviceHub.networkMapCache.notaryIdentities.first()

        // Step 2. Building.
        progressTracker.currentStep = BUILDING
        val newGameState = gameState.addChildPolicy(member)
