...
class ReserveContract  : Contract {
    companion object {
        const val ID = "negotiation.contracts.ReserveContract"
    }
    ...
    override fun verify(tx: LedgerTransaction) {
        ...
        when (cmd.value) {
            is Commands.Propose -> requireThat {
                ...
                "The single output is of type Reservestate" using (tx.outputsOfType<Reservestate>().size == 1)
                "There is exactly one command" using (tx.commands.size == 1)
                "There is no timestamp" using (tx.timeWindow == null)

                val output = tx.outputsOfType<Reservestate>().single()
            ...
            is Commands.Accept -> requireThat {
                ...
                "The single input is of type Reservestate" using (tx.inputsOfType<Reservestate>().size == 1)
                "There is exactly one output" using (tx.outputStates.size == 1)
                "The single output is of type PolicyState" using (tx.outputsOfType<Reservestate>().size == 1)
                "There is exactly one command" using (tx.commands.size == 1)
                "There is no timestamp" using (tx.timeWindow == null)

                val input = tx.inputsOfType<Reservestate>().single()
                val output = tx.outputsOfType<Reservestate>().single()
...
@BelongsToContract(ReserveContract::class)
